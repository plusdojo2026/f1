...
public class UsersDAOTest {
	...
	public static void main(String[] args) {
		...
		List<UsersDTO> userListSel1 = userDao.select(new UsersDTO(0, "", "", "新潟県", "", ""));
		UsersDAOTest.showAllData(userListSel1);
		// select()のテスト2
		System.out.println("---------- select()のテスト2 ----------");
		List<UsersDTO> userListSel2 = userDao.select(new UsersDTO(0, "", "", "", "", ""));
		UsersDAOTest.showAllData(userListSel2);
	
		System.out.println("---------- insert()のテスト ----------");
		UsersDTO insRec = new UsersDTO(0, "f1@gmail.com", "password", "香川県", "", "");
		...
		List<UsersDTO> cardListUp = userDao.select(new UsersDTO(0, "", "", "香川県", "", ""));
		UsersDTO upRec = cardListUp.get(0);
		upRec.setPrefecture_name("熊本県");
		if (userDao.update(upRec)) {
			System.out.println("更新成功！");
			cardListUp = userDao.select(new UsersDTO(0, "", "", "", "", ""));
